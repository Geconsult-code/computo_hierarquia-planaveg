...
(ORR 2025). Como nas classes 1 e 3, **entra a área total dos polígonos, independente do cruzamento com a VS** (decisão de 21/09/2026);
a VS fica como atributo. A classe subtrai as classes 1 (Recooperar), 3 (SICAR-regularização) e 4 (Outros projetos).

## Entrada

| Arquivo | Uso |
|---|---|
| `ORR_Observatorio_Restauracao_2025_com_area.gpkg`, camada `Observatorio_da_Restauracao_2025` | 4 polígonos, um por bioma (Amazônia, Caatinga, Cerrado, Mata Atlântica), já dissolvidos; 38.473 partes; sem Pampa e Pantanal |
...
Características do arquivo: CRS Albers customizado (o WKT "Albers" é lido pelo pyproj, mas não pelo GDAL do QGIS; no QGIS usar
`+proj=aea +lat_0=-12 +lon_0=-54 +lat_1=-2 +lat_2=-22 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs`), geometrias Z/M
(o passo 1 força 2D) e nenhum atributo por projeto. O passo 1 grava os 4 polígonos com `id_proj` = `ORR-Amazonia`, `ORR-Caatinga`, `ORR-Cerrado`
e `ORR-Mata_Atlantica`.
...
| Versão do arquivo | ORR 2025 como entregue (sem Pampa e Pantanal); se sair versão nova, trocar `FONTES['or']` em `config_computo.py` | Adotado |
...
3. **Sobreposição entre os 4 polígonos** (`<p>_sobreposta_na_classe_ha`): os polígonos do ORR são disjuntos entre si (conferido: 0,0 ha).
4. `<p>_area_liquida_ha` = área total - sobreposta às classes anteriores. Os polígonos líquidos são disjuntos entre si e das classes 1, 3 e 4
   (camadas `P2_OR_vs22q` e `P2_OR_vs2224q`, com uma parte por linha).
...
## Conferências (`T5_conferencias.csv`, 22 verificações)

Área geodésica x área declarada no arquivo (dentro de 0,1%: o campo `Area_ha` do ORR veio de projeção equivalente); polígonos do ORR disjuntos; identidade
de área por polígono (inteira = sobreposta às anteriores + na classe + líquida); líquida = união do ORR - união das classes 1, 3 e 4 (cálculo
independente); sobreposição zero entre líquidos e com as classes 1, 3 e 4; células UF x bioma somam a área do polígono; área fora dos limites do IBGE
...
(~3,5 min por versão). Exige as classes 1, 3 e 4 já processadas.
